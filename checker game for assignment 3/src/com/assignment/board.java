...
public class board {
    ...
    public void initial() {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (j % 2 != 0) {
                    board[0][j] = "[R_P]";
                    piece[0][j] = new pieces(1);
                    board[2][j] = "[R_P]";
                    piece[2][j] = new pieces(1);
                    board[6][j] = "[W_P]";
                    piece[6][j] = new pieces(2);
                }
                if (j % 2 == 0) {
                    board[1][j] = "[R_P]";
                    piece[1][j] = new pieces(1);
                    board[5][j] = "[W_P]";
                    piece[5][j] = new pieces(2);
                    board[7][j] = "[W_P]";
                    piece[7][j] = new pieces(2);
                }
            }
        }
    }
    ...
    public void show() {
        this.count(redpieces, whitepieces);
        ...
        System.out.print("\n  +------------------------------------------+");
        System.out.printf("       Red Pieces: %d   White Pieces: %d",rp, wp);
        for (int i = 0; i < 8; i++) {
            System.out.print("\n");
    ...
    public boolean iflegal(int currentx, int currenty, int futurex, int futurey) {
        ...
        if (board[futurex][futurey] == "[R_P]" || board[futurex][futurey] == "[W_P]") {
            islegal = false;
            System.out.printf("1.Move is illegal!\n");
        }
        return islegal;
    }
    ...
    public boolean ifdiagonal(int currentx, int currenty, int futurex, int futurey) {
        ...
        if (xchange == ychange) {
            ...
            System.out.printf("2.The movement is not diagonal\n");
            return false;
        }
    }
    ...
    public boolean ifnsquares(int currentx, int currenty, int futurex, int futurey) {
        ...
        if (nsquares) {
            System.out.printf("3.The movement is illegal\n");
        }
        return nsquares;
    }
    ...
    public boolean ifmoveaway(int currentx, int currenty, int futurex, int futurey) {
        ...
        if (canmoveaway == false) {
            System.out.printf("4.Move is illegal!\n");
        }
        return canmoveaway;
    }
...
//change the type of pieces if possible
    public void playermove(int currentx, int currenty, int futurex, int futurey) {
        ...
        if (board[currentx][currenty] == "[R_B]") {
            board[currentx][currenty] = "[   ]";
            board[futurex][futurey] = "[R_B]";
            piece[currentx][currenty] = new pieces(0);
            piece[futurex][futurey] = new pieces(5);
        }
        if (board[currentx][currenty] == "[W_B]") {
            board[currentx][currenty] = "[   ]";
            board[futurex][futurey] = "[W_B]";
            piece[currentx][currenty] = new pieces(0);
            piece[futurex][futurey] = new pieces(6);
        }
        ...
        if (futurex == 0 && board[futurex][futurey] == "[R_K]") {
            piece[futurex][futurey] = new pieces(5);
        }
        if (futurex == 7 && board[futurex][futurey] == "[W_K]") {
            piece[futurex][futurey] = new pieces(6);
        }
    }
    ...
    public void bombcapturepiece(int currentx,int currenty,int futurex,int futurey){
        if(Math.abs(currentx-futurex)>1){
            if(board[futurex][futurey]=="[R_B]"||board[futurex][futurey]=="[W_B]"){
                if((futurex+1)>=0&&(futurex+1)<=7){
                    if(((futurey-1)>=0)&&((futurey-1)<=7)){
                        board[futurex+1][futurey-1]="[   ]";
                        piece[futurex+1][futurey-1]=new pieces(0);
                    }
                    if(((futurey+1)>=0)&&((futurey+1)<=7)){
                        board[futurex+1][futurey+1]="[   ]";
                        piece[futurex+1][futurey+1]=new pieces(0);
    ...
    public int piececolor(int currentx, int currenty) {
        int colorofpiece = 0;
        if (board[currentx][currenty] == "[R_P]" || board[currentx][currenty] == "[R_K]"||board[currentx][currenty]=="[R_B]") {
            colorofpiece = 1;
        }
        if (board[currentx][currenty] == "[W_P]" || board[currentx][currenty] == "[W_K]"||board[currentx][currenty]=="[W_B]") {
            colorofpiece = 2;
        }
        return colorofpiece;
    }

    public void bombconversion(int futurex, int futurey) {
        if (board[futurex][futurey] == "[R_K]" && futurex == 0) {
            board[futurex][futurey] = "[R_B]";
            piece[futurex][futurey] = new pieces(5);
        }
        if (board[futurex][futurey] == "[W_K]" && futurex == 7) {
            board[futurex][futurey] = "[W_B]";
            piece[futurex][futurey] = new pieces(6);
        }
    }

    public boolean ifbomb(int currentx, int currenty) {
        boolean isbomb = false;
        if (board[currentx][currenty] == "[R_B]" || board[currentx][currenty] == "[W_B]") {
            isbomb = true;
        }
        return isbomb;
    }

    public void count(int redpieces, int whitepieces){
        rp = 0;
        wp = 0;
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (board[i][j] == "[R_P]" || board[i][j] == "[R_K]" || board[i][j] == "[R_B]") {
                    rp++;
                }
                if (board[i][j] == "[W_P]" || board[i][j] == "[W_K]" || board[i][j] == "[W_B]") {
                    wp++;
                }
            }
        }
